...
<div class="card border-0 shadow-sm mb-4" style="border-radius:6px">
    <div class="card-body p-3">
        <form method="GET" class="d-flex gap-2 flex-wrap">
            <input type="text" name="search" class="form-control" style="max-width:220px;border-radius:4px" placeholder="Rechercher des produits..." value="{{ request('search') }}">
            ...
            @if(request()->hasAny(['search','category']))
                <a href="{{ route('admin.products.index') }}" class="btn btn-outline-secondary">Effacer</a>
            @endif
        </form>
    </div>
</div>
...
<div class="card border-0 shadow-sm" style="border-radius:6px">
    <div class="table-responsive">
        <table class="table mb-0">
            <thead><tr>
                <th>Produit</th><th>Catégorie</th><th>Prix</th><th>Stock</th><th>En vedette</th><th>Actif</th><th>Actions</th>
            </tr></thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($product->sale_price)
                            <span style="color:#c4786a">DH{{ number_format($product->sale_price,2) }}</span>
                            <small class="text-muted ms-1" style="text-decoration:line-through">DH{{ number_format($product->price,2) }}</small>
                        @else
                            DH{{ number_format($product->price,2) }}
                        @endif
                    </td>
                    <td>
                    ...
                    <td>
                        <a href="{{ route('admin.products.edit', $product) }}" class="btn btn-sm btn-outline-secondary me-1">Modifier</a>
